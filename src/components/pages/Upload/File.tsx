  ...
  const handleChunkedFiles = async (expiresAt: Date, toChunkFiles: File[]) => {
    for (let i = 0; i !== toChunkFiles.length; ++i) {
      ...
      for (let j = 0; j !== chunks.length; ++j) {
              ...
              if (j === chunks.length - 1) {
                ...
                if (!navigator.clipboard)
                  showNotification({
                    ...
                    message: 'MoreLoad is unable to copy to clipboard due to security reasons.',
                    color: 'red',
                  });
              }
        ...
        req.setRequestHeader('X-MoreLoad-Partial-FileName', file.name);
        req.setRequestHeader('X-MoreLoad-Partial-MimeType', file.type);
        req.setRequestHeader('X-MoreLoad-Partial-Identifier', identifier);
        req.setRequestHeader('X-MoreLoad-Partial-LastChunk', j === chunks.length - 1 ? 'true' : 'false');
