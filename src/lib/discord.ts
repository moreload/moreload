...
export async function sendUpload(user: User, file: File, raw_link: string, link: string) {
  ...
  const body = JSON.stringify({
    username: (config.discord.username ?? config.discord.upload.username) || 'MoreLoad',
    avatar_url:
      (config.discord.avatar_url ?? config.discord.upload.avatar_url) ||
      'https://raw.githubusercontent.com/moreload/moreload/main/public/moreload.png',
...
export async function sendShorten(user: User, url: Url, link: string) {
  ...
  const body = JSON.stringify({
    username: (config.discord.username ?? config.discord.shorten.username) || 'MoreLoad',
    avatar_url:
