...
import { NextApiReq, NextApiRes, withMoreLoad } from 'lib/middleware/withMoreLoad';
...
async function handler(req: NextApiReq, res: NextApiRes) {
  ...
  if (req.headers['content-range']) {
    ...
    const filename = req.headers['x-moreload-partial-filename'] as string;
    const mimetype = req.headers['x-moreload-partial-mimetype'] as string;
    const identifier = req.headers['x-moreload-partial-identifier'];
    const lastchunk = req.headers['x-moreload-partial-lastchunk'] === 'true';
    ...
    const tempFile = join(tmpdir(), `moreload_partial_${identifier}_${start}_${end}`);
    ...
    if (lastchunk) {
      const partials = await readdir(tmpdir()).then((files) =>
        files.filter((x) => x.startsWith(`moreload_partial_${identifier}`))
      );
  ...
  for (let i = 0; i !== req.files.length; ++i) {
    ...
    switch (format) {
      ...
      case FileNameFormat.NAME:
        ...
        break;
      default:
        if (req.headers['x-moreload-filename']) {
          fileName = req.headers['x-moreload-filename'] as string;
